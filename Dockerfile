...
RUN apt-get update && apt-get install -y libpq-dev gcc python-dev supervisor nginx openssl openssh-server curl && \
...
CMD ["celery", "-A", "tasks.tasks", "worker", "--loglevel=info", "--queues", "etd_submission_ready"]
